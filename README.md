...
<img src="assets/logo.svg" alt="flow" width="340">
...
outcome to your branch with no human in the loop. The core has zero third-party dependencies.
...
![runtime deps](https://img.shields.io/badge/runtime%20deps-0-7C3AED)
...
Every number here is measured and reproducible from this repo.

| Result                        | What it means                                                                                                   |
| :---------------------------- | :-------------------------------------------------------------------------------------------------------------- |
| **0.75 zero-touch ship rate** | a real 4-task backlog produced 3 merged commits on `main`, suite green (8 to 14 tests), with zero human touches |
| **83.5% fewer tokens**        | a fan-out that died at 906k tokens completed at 135k under the _same_ ceiling, flipping `failed` to `completed` |
| **`kill -9`, then resume**    | SIGKILL mid-run, `flow resume`, and completed work never re-runs (proven by real subprocess tests)              |
| **205 hermetic tests**        | green in CI with no network; live provider routes are credential-gated                                          |

## The loop most agent frameworks never close

Spin up parallel sub-agents and you get speed, plus a mess. Loose patches you still have to
integrate by hand. A model grading its own homework. A surprise bill at 3 a.m. A run that
dies on a rate limit and starts over from zero.
...
- **It proves results instead of trusting them.** JSON schema at every leaf. A _different_
  model as the verifier, with the loop refusing to start if the verifier is the executor.
  Real test and build commands as merge gates, so green receipts land on disk or nothing ships.
- **It cannot overspend.** Token, dollar, call, and deadline ceilings are enforced _before_
  every call and replayed across crash-restarts. Set one flag and the run physically cannot
  exceed it.
- **It survives anything.** An fsync'd write-ahead journal means a reboot, a `kill -9`, or a
  rate-limit death costs you nothing. `flow resume` skips every finished leaf.
- **It ships.** Coding agents run in isolated checkouts and hand back patches. flow proof-gates
  them, repairs conflicts, and auto-merges to your branch behind a fail-closed allowlist, then
  auto-reverts if a post-merge canary goes red.

You write one Python function. Your code owns the control flow; models do the bounded work you
hand them. That inversion is the whole design.

## Watch it ship

A backlog goes in. Merged, proven commits come out, with nobody watching.
...
Run against a real repo, this shipped **3 of 4 tasks straight to `main`**. Each one applied,
tested, and committed with no human in the loop, while the 4th was _exiled_ on an unresolvable
conflict instead of force-merged. The test suite grew from 8 to 14 and stayed green.

Autonomous merge is **gated, not trusted**. A patch that edits an existing test is sent to a
cross-vendor reviewer and fails closed. A flaky check is quarantined, not believed. Auto-merge
fires only for an allowlisted repo. A red canary reverts the merge and exiles the batch.
[Full guide.](docs/merge.md)
...
flowchart LR
    ...
    ENGINE --> A1["Codex CLI<br/>Claude Code<br/>any agent harness"]
    ENGINE --> J[("journal<br/>(crash-proof)")]
    J -.->|"flow resume"| ENGINE
```

Any model, any provider. Use an API key, or a subscription you already have (ChatGPT Pro,
Claude Max). The entire core runtime is the Python standard library.
...
| :------------------------------------------ | :--------------------------------------------------------------------------------------------------------------------------- |
| Fan out 20 checks, lenses, or files at once | `wf.parallel`, `wf.pipeline`, with real concurrency and per-leaf failure policy                                              |
| Trust model output                          | JSON Schema at every leaf; malformed output is self-repaired with the exact error, then fails loudly                         |
| Never get a surprise bill                   | hard token, USD, call, and deadline budgets, enforced _before_ each call and across crash-restarts                           |
| Iterate until the result is actually good   | `wf.loop`: bounded retry-until-verified with a _different_ model as the judge, no self-grading                               |
| Stop paying N times for the same context    | `wf.block`: store shared context once, siblings get a reference (fan-outs measured 53 to 89% redundant without it)           |
| Survive crashes, reboots, rate-limit deaths | fsync'd journal; `flow resume` skips all completed work                                                                      |
| Fan out real coding agents in parallel      | `wf.code` with `isolation="worktree"`: N agents in isolated checkouts, changes returned as patches, your live repo untouched |
...
Reach for something simpler for one-shot prompts or deterministic ETL. Workflows multiply
calls, so always set a budget.
...
_different_ model accepts it, and stops itself when it is stuck, broke, or done.
...
the executor), **deterministic stops** (`goal_met`, then `max_iterations`, then `no_progress`,
then `budget_exhausted`, then `cancelled`), **stall detection** that halts on repeated evidence
_before_ the token wall, and a **handoff report** on every stop with a concrete next action,
never a bare `"failed"`. Completed iterations never re-run after a crash. [Full guide.](docs/loop.md)

## Don't pay N times for the same context

Wide fan-out's hidden cost is every sibling re-ingesting the same context, the dominant spend
at scale (real runs: **53 to 89% redundant input**).
...
**Receipt:** a 25-leaf research run that died at 906,313 of 900,000 tokens was replayed under
the _same_ ceiling and completed at 205,073 (input 822,120 down to 135,619, **83.5% fewer**),
stopping on `goal_met` via an independent verifier. The dedup rate is _measured_ from per-leaf
input hashes, not estimated.

## Coding agents as leaves

Each leaf can be a **full CLI coding agent**, whether Codex, Claude Code, or any harness, that
uses its own tools (edit files, run commands, own a workspace) and returns a structured receipt
with real spend, a continuation handle, and patch evidence. All of it stays inside the budget
gate, the crash-resume journal, and schema enforcement.
...
Harness behavior lives in config, not per-leaf code. [Full guide.](docs/agents.md)

#### vs Claude Code Workflow, honest

|                                       | flow                                          | Claude Code Workflow |
| :------------------------------------ | :-------------------------------------------- | :------------------- |
| Vendor                                | any (Codex, Claude, generic)                  | Claude only          |
| Pre-spend budget gate                 | yes, kill-proof, replays across restarts      | varies               |
| Parallel agent mutation, same repo    | safe (isolated checkouts plus patch evidence) | not designed for it  |
| Crash resume                          | yes, a completed agent leaf never re-runs     | session-bound        |
| Independent cross-vendor verifier     | yes (`wf.loop`)                               | manual               |
| **Proof-gated auto-merge and revert** | **yes (`wf.merge`)**                          | **none**             |
| Interactive UI                        | `flow watch` (live progress)                  | richer built-in UI   |

CC Workflow's leaves are first-class CC subagents with a nicer interactive UI. flow's ground is
everywhere else: any vendor, hard budgets, crash resume, and the entire **prove, integrate,
ship** layer it has no equivalent for.
...
flow run --nl "<task>"            # model authors the workflow, flow validates and runs it
...
| `codex`         | ChatGPT-subscription Codex route                                            | fails closed |
...
Models, tiers, and pricing are pure config: one JSON or YAML file, no code.
...
cancellation, schema repair, crash-resume via real `SIGKILL` subprocesses, budget replay across
restarts, loop stops and gates and repair routing and handoffs, deterministic stop replay, block
dedup, verifier-identity enforcement, the full merge orchestrator (proof gates, the three
guards, money fence, auto-revert), agentic leaves, and a v1 regression lock. A replay harness
re-runs real failed production manifests under their original ceilings as the performance bar.
